...
//   $files->debugsql = 1;
  ...
  for ($i=0;$i<$files->getRows();$i++)
  {
    ...
    $file_mcat         = $files->getValue('category_id');
    ...
    {
      $thumbnail = '<img src="'. $loading .'" longdesc="media/mime-error.gif" width="44" height="38" alt="file does not exist" />';
    }
    else
    {
      ...
      $thumbnail = '<img src="'. $loading .'" longdesc="'. $icon_src .'" width="44" height="38" alt="'. $alt .'" title="'. $alt .'" />';

      if (OOMedia::_isImage($file_name) && $thumbs)
      {
        $thumbnail = '<img src="'.$loading.'" class="" longdesc="'. $REX['HTDOCS_PATH'] .$REX['MEDIA_DIR'].'/'.$file_name.'" width="80" alt="'. $alt .'" title="'. $alt .'" />';
        if ($image_manager)
        {
          if($file_mcat == $rex_file_category) {
            $loading = $REX['HTDOCS_PATH'] . $REX['FRONTEND_FILE'] .'?rex_img_type=rex_mediapool_preview&amp;rex_img_file='.$encoded_file_name;
          }
          $thumbnail = '<img src="'.$loading.'" class="" longdesc="'. $REX['HTDOCS_PATH'] . $REX['FRONTEND_FILE'] .'?rex_img_type=rex_mediapool_preview&amp;rex_img_file='.$encoded_file_name.'" alt="'. $alt .'" title="'. $alt .'" />';
        }else if($image_resize)
        {
          if($file_mcat == $rex_file_category) {
            $loading = $REX['HTDOCS_PATH'] . $REX['FRONTEND_FILE'] .'?rex_resize=80a__'.$encoded_file_name;
          }
          $thumbnail = '<img src="'.$loading.'" class="" longdesc="'. $REX['HTDOCS_PATH'] . $REX['FRONTEND_FILE'] .'?rex_resize=80a__'.$encoded_file_name.'" alt="'. $alt .'" title="'. $alt .'" />';
        }
      }
    }
    ...
    $display = ($file_mcat == $rex_file_category) ? 'table-row' : 'none';
    echo '<tr class="file mcat_'.$file_mcat.'" style="display:'.$display.';">
            '. $add_td .'
...
<script>
(function($){
  $('#rex_file_category').bind('change', function(){
    var mcat = $(this).val();
    $('tr.file').hide();
    $('tr.mcat_'+mcat).show();
    $('tr.mcat_'+mcat).each(function(){
      img = $(this).children('td.rex-thumbnail').children('a').children('img');
      img.attr('src', img.attr('longdesc'));
    });
    $('.rex-message-block').remove();
  })
})(jQuery)
</script>
